...
describe("provider matrix — honest taxonomy (truth dashboard)", () => {
  ...
  it("derives the beta verdict from explicit capability checks", () => {
    ...
    // The bundled project-owned dataset only carries automated-checks-passed.
    // A closed beta requires human editorial review to be recorded.
    expect(checks.symbolContentReady).toBe(false);
