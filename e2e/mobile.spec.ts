...
test("birth form works end-to-end at 390px", async ({ page }) => {
  ...
  await page.getByLabel("Name or nickname").fill("Mobile Test");
  ...
  await page.getByLabel("Birthplace", { exact: true }).fill("Paris");
  await expect(page.getByRole("option", { name: /Paris/ }).first()).toBeVisible({ timeout: 15_000 });
